...
def debug_and_clean_views():
    ...
    if try_count > except_count:
        warnings.append(
            f"Some try blocks missing except: {try_count} try, {except_count} except"
        )
        print(f"  ⚠️ {try_count} try blocks, {except_count} except blocks")
    else:
        print(
            f"  ✅ Error handling looks good ({try_count} try, {except_count} except)"
        )
    ...
    functions_with_docstrings = re.findall(
        r'def (\w+)\(.*?\):\s*"""[^"]*"""', content, re.DOTALL
    )

    if len(functions) > len(functions_with_docstrings):
        warnings.append(
            f"{len(functions) - len(functions_with_docstrings)} functions missing docstrings"
        )
        print(
            f"  ⚠️ {len(functions) - len(functions_with_docstrings)} functions missing docstrings"
        )
    else:
    ...
    if missing_decorators:
        warnings.append(
            f"Views missing @login_required: {', '.join(missing_decorators)}"
        )
    ...
    if api_views:
        for api_view in api_views:
            if f"def {api_view}" in content:
                ...
                method_check = re.search(
                    f"def {api_view}.*?if request.method", content, re.DOTALL
                )
                if not method_check:
    ...
    json_without_status = len(
        re.findall(r"JsonResponse\([^)]*\)(?!.*status=)", content)
    )

    if json_without_status > json_with_status:
        warnings.append(
            f"Some JsonResponse missing status codes ({json_without_status} without status)"
        )
