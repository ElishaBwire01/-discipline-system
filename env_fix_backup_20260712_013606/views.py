        ...
        if stream_id and form:
            try:
                ...
                if conflict:
                    messages.error(
                        request,
                        f"{stream.name} - {form} already has an approved class teacher.",
                    )
                else:
...
def admin_dashboard(request):
    ...
    class_teacher_assignments = (
        ...
        .select_related("user", "assigned_stream")
        .order_by("assigned_stream__name", "assigned_form")
    )
        ...
        if action == "add_student":
            try:
                ...
                if Student.objects.filter(admission_number=admission_number).exists():
                    messages.error(
                        request, f"Admission number {admission_number} already exists."
                    )
                    return redirect("/admin-dashboard/")
...
def class_teacher_dashboard(request):
    ...
    if not assigned_form:
        messages.error(
            request, "No form/grade assigned to your profile yet. Please contact admin."
        )
        return redirect("choose_stream")
    ...
    if request.method == "POST":
        ...
        if action == "report_student":
            ...
            try:
                ...
                messages.success(
                    request,
                    f"Student added to {assigned_stream.name} - {assigned_form}!",
                )
            except (ValueError, TypeError) as e:
                messages.error(request, f"Error adding student: {e}")
            return redirect("/class-teacher-dashboard/")
    ...
    my_reports = (
        DisciplineReport.objects.filter(student__in=display_students)
...
def class_recommendations(request, stream_id):
    try:
        ...
        if critical > 0:
            critical_names = ", ".join(
                students.filter(risk_level="CRITICAL").values_list("name", flat=True)[
                    :5
                ]
            )
            recommendations.append(
        ...
        if warning > 3:
            ...
            warning_names = ", ".join(
                students.filter(risk_level="WARNING").values_list("name", flat=True)[:3]
            )
            recommendations.append(
                {
                    "type": "WARNING",
...
def approve_reset(request, reset_id):
    ...
    if request.method == "POST":
        ...
        notification.target_users.add(user)
        messages.success(
            request, f"Password reset for {user.username}. New password: {new_password}"
        )
...
def global_recommendations(request):
    ...
    try:
        ...
        if critical_count > 0:
            student_names = ", ".join(
                s.name for s in students.filter(risk_level="CRITICAL")[:5]
            )
            recommendations.append(
                {
        ...
        if warning_count > 0:
            warning_names = ", ".join(
                s.name for s in students.filter(risk_level="WARNING")[:3]
            )
            recommendations.append(
                {
