...
def profile_picture_html(user, request):
    ...
    if (
        ...
        and user.teacher_profile.profile_picture
    ):
        try:
            url = request.build_absolute_uri(user.teacher_profile.profile_picture.url)
            return mark_safe(
                f'<img src="{url}" alt="{user.username}" class="teacher-avatar" onerror="this.style.display=\'none\';">'
            )
        except:
            pass
    initials = (
        user.get_full_name()[:1].upper()
        if user.get_full_name()
        else user.username[:1].upper()
    )
