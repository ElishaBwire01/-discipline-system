...
from django.db.models import Count, Q, Avg, Max, Min, Sum  # Added Sum
...
def class_teacher_required(user):
    """Check if user is a class teacher or superuser"""
    return user.is_superuser or user.groups.filter(name="ClassTeacher").exists()


def get_teacher_profile(user):
    """Get teacher profile for a user, returns None if not exists"""
    try:
...
def _create_report(request, student, post):
    ...
    """
    from django.contrib import messages
    from .models import DisciplineCategory, DisciplineReport
    ...
    # Update category name if custom
    if custom_case:
...
def _get_form_choices():
    ...
    from .models import GradeLevel, Student
...
def get_risk_color(risk_level):
    ...
    return colors.get(risk_level, "info")
...
def get_risk_badge(risk_level):
    ...
    color = colors.get(risk_level, "info")
    icon = icons.get(risk_level, "fa-info-circle")
    return f'<span class="badge-modern badge-{color}"><i class="fas {icon}"></i> {risk_level}</span>'


def get_student_summary(student):
    """
    Get a summary dictionary for a student
    """
    reports_count = student.reports.count()
    total_points = student.reports.aggregate(total=Sum("points"))["total"] or 0
...
def create_notification_for_teachers(
    student, title, message, notification_type="warning"
):
    """
    Create notification for all class teachers of a student's stream
    """
    from .models import Notification, User

    class_teachers = User.objects.filter(
        teacher_profile__assigned_stream=student.stream, groups__name="ClassTeacher"
    )
...
def create_notification_for_admins(
    title, message, notification_type="critical", student=None
):
    """
...
def calculate_risk_trend(student):
    """
    Calculate risk trend for a student based on recent reports
    ...
    points = [r.points for r in reports]
...
def validate_admission_number(admission_number):
    ...
    # Example: Format should be like 'STU-2024-001'
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if role == "class_teacher":
            ...
            profile.save()
            messages.success(
                request, "Registration successful! Waiting for admin approval."
            )
        else:
...
def choose_stream(request):
    ...
    if not profile:
        profile = TeacherProfile.objects.create(
            user=request.user, has_chosen_stream=False
        )
    ...
    if request.method == "POST":
        ...
        if stream_id:
            try:
                ...
                profile.save()
                messages.success(
                    request, "Stream assigned! Waiting for admin approval."
                )
                return redirect("dashboard")
...
def admin_dashboard(request):
    try:
        ...
        warning_count = Student.objects.filter(
            is_active=True, risk_score__range=(30, 59)
        ).count()
        critical_count = Student.objects.filter(
            is_active=True, risk_score__gte=60
        ).count()

        good_percentage = (
            round(good_count / total_students * 100, 1) if total_students else 0
        )
        warning_percentage = (
            round(warning_count / total_students * 100, 1) if total_students else 0
        )
        critical_percentage = (
            round(critical_count / total_students * 100, 1) if total_students else 0
        )

        high_report_students = (
        ...
        online_teachers = TeacherProfile.objects.filter(is_online=True).select_related(
            "user"
        )
        ...
        # -- Pending Approvals & Resets --
        pending_approvals = TeacherProfile.objects.filter(
            is_approved=False, is_suspended=False, has_chosen_stream=True
        ).count()
            ...
            if action == "add_student":
                try:
                    ...
                    if not admission_number or not name:
                        messages.error(
                            request, "Admission number and name are required."
                        )
                        return redirect("admin_dashboard")
...
def class_teacher_dashboard(request):
    ...
    try:
        assigned_stream = Stream.objects.get(
            id=profile.assigned_stream_id, is_active=True
        )
    except Stream.DoesNotExist:
        ...
        profile.save()
        messages.error(
            request, "? Your assigned stream was deleted. Please choose a new stream."
        )
        return redirect("choose_stream")
    ...
    # -- Student Queryset --
    display_students = (
        Student.objects.filter(is_active=True, stream=assigned_stream)
        .select_related("stream")
        .order_by("name")
    )
    ...
    my_reports = (
        DisciplineReport.objects.filter(student__stream=assigned_stream)
        .select_related("student", "reported_by", "category")
...
def teacher_dashboard(request):
    ...
    students_qs = (
        Student.objects.filter(is_active=True).select_related("stream").order_by("name")
    )
...
def student_profile(request, student_id):
    """
    Display student profile with reports and AI insights
    """
    student = get_object_or_404(Student, id=student_id, is_active=True)
    ...
    if reports.exists():
        running_score = 0
        for report in reports.order_by("reported_at"):
            running_score += report.points
            timeline.append(
                {"date": report.reported_at, "score": min(running_score, 100)}
            )
    ...
    if request.method == "POST":
        ...
        if action == "update_student":
            ...
            if request.user.is_superuser:
                student.delete()
                messages.success(
                    request, f"? Student {student_name} permanently deleted."
                )
            else:
...
def search_students_api(request):
    """
    API endpoint to search students by name or admission number
    """
...
def online_teachers_api(request):
    """
    API endpoint to get online teachers
    """
...
def student_by_name(request):
    """
    API endpoint to get student by name
    """
    try:
...
def student_by_admission(request, admission_number):
    """
    API endpoint to get student by admission number
    """
    try:
...
def get_categories_api(request):
    """
    API endpoint to get all discipline categories
    """
    try:
...
def global_recommendations(request):
    try:
        ...
        if warning_count > 0:
            warning_qs = students.filter(risk_level="WARNING").order_by("-risk_score")[
                :3
            ]
        ...
        improving_students = students.filter(
            risk_level="WARNING", intervention_count__gt=0
        ).count()
        ...
        no_recent_incidents = (
            students.filter(last_incident_date__isnull=True).count()
            if hasattr(Student, "last_incident_date")
            else 0
        )
...
def student_recommendations(request, student_id):
    try:
        ...
        if total_reports > 0:
            for cat in reports.values("category__name").annotate(count=Count("id")):
                if cat["count"] >= 2:
                    recommendations.append(
                        {
                            "type": "pattern",
...
def class_recommendations(request, class_id):
    try:
        ...
        if critical > 0:
            critical_students_list = students.filter(risk_level="CRITICAL").values_list(
                "name", flat=True
            )[:5]
        ...
        if warning > 3:
            ...
            warning_students_list = students.filter(risk_level="WARNING").values_list(
                "name", flat=True
            )[:3]
            ...
            all_students_data.append(
                {
                    ...
                    "days_since_incident": (
                        student.days_since_last_incident
                        if hasattr(student, "days_since_last_incident")
                        else None
                    ),
                }
            )
...
def ai_risk_stats(request):
    ...
    try:
        ...
        stats = {
            ...
            "warning_count": students.filter(risk_level="WARNING").count(),
            "good_count": students.filter(risk_level="GOOD").count(),
            "avg_risk_score": students.aggregate(Avg("risk_score"))["risk_score__avg"]
            or 0,
            "max_risk_score": students.aggregate(Max("risk_score"))["risk_score__max"]
            or 0,
            "min_risk_score": students.aggregate(Min("risk_score"))["risk_score__min"]
            or 0,
            "total_reports": DisciplineReport.objects.count(),
            "reports_today": DisciplineReport.objects.filter(
                reported_at__date=timezone.now().date()
            ...
            "total_interventions": sum(s.intervention_count for s in students),
            "students_with_interventions": students.filter(
                intervention_count__gt=0
            ).count(),
        }
    ...
    if request.method == "POST" and request.FILES.get("excel_file"):
        try:
            ...
            if not excel_file.name.lower().endswith((".xlsx", ".xls")):
                messages.error(
                    request, "? Please upload a valid Excel file (.xlsx or .xls)."
                )
                return redirect("admin_dashboard")
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    if not name or not admission:
                        errors.append(
                            f"Row {index + 2}: Missing name or admission number."
                        )
                        continue
                    ...
                    if created:
                        ...
                        errors.append(
                            f'Row {index + 2}: Student "{admission}" already exists.'
                        )
        ...
        for student in students:
            ...
            if total_reports >= 2:
                ...
                if first_report and last_report:
                    days_diff = (
                        last_report.reported_at - first_report.reported_at
                    ).days
                    if days_diff > 0:
...
def ai_behavior_patterns(request):
    ...
    try:
        ...
        student_patterns = (
            reports.values("student__name", "student__stream__name")
            .annotate(count=Count("id"))
            .filter(count__gte=3)
            .order_by("-count")[:10]
        )
        ...
        for student in multi_report_students[:20]:
            categories = student.reports.values_list(
        ...
        for student in Student.objects.filter(is_active=True):
            ...
            if reports_qs.count() >= 2:
                first_risk = (
                    reports_qs.first().student.risk_level
                    if reports_qs.first().student
                    else "GOOD"
                )
                last_risk = (
                    reports_qs.last().student.risk_level
                    if reports_qs.last().student
                    else "GOOD"
                )
                    ...
                    risk_transitions.append(
                        {
                            ...
                            "to_risk": last_risk,
                            "improved": last_risk == "GOOD"
                            or (last_risk == "WARNING" and first_risk == "CRITICAL"),
                        }
                    )
...
def ai_intervention_effectiveness(request):
    ...
    try:
        ...
        for student in students_with_interventions:
            ...
            before_avg = (
                sum(r.points for r in before_reports) / before_reports.count()
                if before_reports.count() > 0
                else 0
            )
            after_avg = (
                sum(r.points for r in after_reports) / after_reports.count()
                if after_reports.count() > 0
                else 0
            )
        ...
        effectiveness_data.sort(
            key=lambda x: (
                0
                if x["effectiveness"] == "High"
                else (
                    1
                    if x["effectiveness"] == "Medium"
                    else 2 if x["effectiveness"] == "Moderate" else 3
                )
            )
        )
...
def request_password_reset(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render(
                request, "request_reset.html", {"error": "Username not found"}
            )
...
def reject_reset(request, reset_id):
    if request.method == "POST":
        ...
        reset.save()
        messages.success(
            request, f"? Reset request for {reset.user.username} rejected."
        )
    return redirect("admin_reset_requests")
    ...
    if request.method == "POST":
        ...
        if action == "change_password":
            ...
            if not user.check_password(current_password):
                ...
                User.objects.exclude(id=user.id).filter(username=new_username).exists()
            ):
