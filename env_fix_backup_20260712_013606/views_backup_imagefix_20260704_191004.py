...
from django.db.models.functions import (
    ExtractWeekDay,
)  # ADD THIS - needed for weekday extraction
import json
import os
import pandas as pd
from .models import (
...
def admin_dashboard(request):
        ...
        if action == "add_student":
            try:
                stream = get_object_or_404(
                    Stream, id=request.POST.get("stream"), is_active=True
                )
    ...
    # -- Context --
    stream_choices = [
        (s.id, s.name) for s in Stream.objects.filter(is_active=True).order_by("name")
    ]
    ...
    if request.method == "POST":
        ...
        if action == "report_student":
            ...
            try:
                ...
                messages.success(
                    request, f"? Student added to {assigned_stream.name}!"
                )
            except Exception as e:
                messages.error(request, f"? Error adding student: {e}")
            return redirect("class_teacher_dashboard")
...
def student_recommendations(request, student_id):
    try:
        ...
        student_data = {
            ...
            "risk_trend": (
                student.risk_trend if hasattr(student, "risk_trend") else "stable"
            ),
        }
