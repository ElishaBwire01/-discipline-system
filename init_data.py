...
from core.models import DisciplineCategory, Stream, TeacherProfile
...
def init():
    ...
    for key, name, points in categories:
        DisciplineCategory.objects.get_or_create(
            key=key, defaults={"name": name, "points": points}
        )
    ...
    if not User.objects.filter(is_superuser=True).exists():
        admin = User.objects.create_superuser(
            username="admin", email="admin@school.com", password="Admin@2025#"
        )
    ...
    print("   Users: 1 (admin)")
    print("\n??  No demo users created. All users must register.")
