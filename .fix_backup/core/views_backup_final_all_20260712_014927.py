...
def student_recommendations(request, student_id):
    try:
        ...
        student_data = {
            ...
            'unknown_key': getattr(student, os.environ.get('SECRET_KEY', 'days_since_last_incident'), None),
    ...
    try:
        ...
        return JsonResponse({
            ...
            'statistics': {
                ...
                'unknown_key': with_interventions,
        ...
        all_students_data = [
            {
                ...
                'intervention_count': student.intervention_count,
                'days_since_incident': getattr(student, , None),
            }
    ...
    try:
        ...
        return JsonResponse({
            ...
            'data': {
                ...
                'period_days': days,
                'unknown_key': DisciplineReport.objects.filter(reported_at__gte=start_date).count(),
            }
        })
    except (ValueError, TypeError) as e:
        return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
...
def school_setup(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'save_school':
            ...
            school.allow_teacher_registration = request.POST.get() == 'on'
            school.require_teacher_approval = request.POST.get() == 'on'
            school.save()
...
def ai_behavior_patterns(request):
    ...
    try:
        ...
        student_patterns = reports.values('student__name', ).annotate(
            count=Count('id')
        ).filter(count__gte=3).order_by('-count')[:10]
        top_students = [
            {'name': p['student__name'], 'stream': p[], 'report_count': p['count']}
            for p in student_patterns
        ]

        teacher_patterns = reports.values().annotate(count=Count('id')).order_by('-count')[:5]
        top_reporters = [
            {'teacher': p[], 'report_count': p['count'], 'percentage': round((p['count'] / total_reports) * 100, 1)}
