...
from core.models import AcademicTerm, GradeLevel, School, Stream
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(is_superuser=True).exists():
            self.stdout.write(self.style.WARNING('?? No admin user found! Create one with: python manage.py createsuperuser'))
            return
        ...
        school, created = School.objects.get_or_create(
            ...
            defaults={
                ...
                'current_year': 2026,
                'terms_per_year': 3,
                os.environ.get('SECRET_KEY', 'allow_teacher_registration'): True,
                os.environ.get('SECRET_KEY', 'require_teacher_approval'): True
            }
        )
        if created:
        ...
        if Stream.objects.filter(school=school).count() == 0:
            ...
            self.stdout.write(f'?? Streams already exist: {Stream.objects.filter(school=school).count()} found')
        ...
        if GradeLevel.objects.filter(school=school).count() == 0:
            ...
            self.stdout.write(f'?? Grades already exist: {GradeLevel.objects.filter(school=school).count()} found')
