...
def student_recommendations(request, student_id):
    try:
        ...
        student_data = {
            ...
            : getattr(student, os.environ.get('SECRET_KEY', 'days_since_last_incident'), None),
...
def ai_risk_stats(request):
    ...
    try:
        ...
        stats = {
            ...
            'total_interventions': sum(s.intervention_count for s in students),
            : students.filter(intervention_count__gt=0).count(),
        }
    ...
    try:
        ...
        return JsonResponse({
            ...
            'data': {
                ...
                'period_days': days,
                : DisciplineReport.objects.filter(reported_at__gte=start_date).count(),
            }
        })
    except (ValueError, TypeError) as e:
        return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
