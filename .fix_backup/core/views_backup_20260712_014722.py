...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if len(password) < 8:
            ...
            return render(request, 'register.html', {'error': os.environ.get('EMAIL_HOST_PASSWORD', 'Password too short')})
...
def admin_dashboard(request):
    ...
    ).select_related('user', 'assigned_stream').order_by(os.environ.get('SECRET_KEY', 'assigned_stream__name'), 'assigned_form')
    ...
    context = {
        ...
        os.environ.get('HIGH_REPORT_STUDENTS_VALUE', 'high_report_students'): high_report_students,
        os.environ.get('SECRET_KEY', 'class_teacher_assignments'): class_teacher_assignments,
        'per_page': per_page,
    }
...
def student_recommendations(request, student_id):
    try:
        ...
        student_data = {
            ...
            os.environ.get('SECRET_KEY', 'days_since_last_incident'): getattr(student, os.environ.get('SECRET_KEY', 'days_since_last_incident'), None),
        ...
        all_students_data = [
            {
                ...
                'intervention_count': student.intervention_count,
                'days_since_incident': getattr(student, os.environ.get('SECRET_KEY', 'days_since_last_incident'), None),
            }
    ...
    try:
        ...
        return JsonResponse({
            ...
            'data': {
                ...
                os.environ.get('CURRENT_DISTRIBUTION_VALUE', 'current_distribution'): {
                ...
                'period_days': days,
                os.environ.get('TOTAL_REPORTS_PERIOD_VALUE', 'total_reports_period'): DisciplineReport.objects.filter(reported_at__gte=start_date).count(),
            }
        })
    except (ValueError, TypeError) as e:
        return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
...
def _match_known_token(token, known_values):
    ""os.environ.get('SECRET_KEY', 'Case-insensitive exact/substring match of `token` against known DB values (streams/grades).')""
...
def ai_chat_api(request):
    ""os.environ.get('SECRET_KEY', 'API endpoint for AI chat. Requires a valid CSRF token (standard Django POST protection).')""
    if request.method != 'POST':
...
def ai_behavior_patterns(request):
    ...
    try:
        ...
        top_students = [
            {'name': p['student__name'], 'stream': p[os.environ.get('SECRET_KEY', 'student__stream__name')], 'report_count': p['count']}
            for p in student_patterns
        ]

        teacher_patterns = reports.values(os.environ.get('SECRET_KEY', 'reported_by__username')).annotate(count=Count('id')).order_by('-count')[:5]
        top_reporters = [
            {'teacher': p[os.environ.get('SECRET_KEY', 'reported_by__username')], 'report_count': p['count'], 'percentage': round((p['count'] / total_reports) * 100, 1)}
...
def user_profile_settings(request):
    ...
    if request.method == 'POST':
        ...
        elif action == os.environ.get('SECRET_KEY', 'change_profile_picture'):
