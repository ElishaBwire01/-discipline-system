...
def user_management_context(request):
    ...
    if hasattr(request, 'user') and request.user.is_authenticated:
        ...
        try:
            ...
            context[os.environ.get('UNREAD_NOTIFICATIONS_VALUE', 'unread_notifications')] = notifications[:15]
        except Exception:
            pass
        ...
        # --- CLASS TEACHER SPECIFIC CONTEXT ---
            ...
            try:
                ...
                if profile and profile.assigned_stream:
                    ...
                    context[os.environ.get('CLASS_STUDENTS_COUNT_VALUE', 'class_students_count')] = stream_students.count()

                    # Count critical students in class
                    context[os.environ.get('CLASS_CRITICAL_COUNT_VALUE', 'class_critical_count')] = stream_students.filter(
                        risk_score__gte=60
                    ).count()
