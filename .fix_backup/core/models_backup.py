...
class DisciplineReport(models.Model):
    ...
    def _notify_admin(self):
        ...
        if admins.exists():
            notification = Notification.objects.create(
                title=f'?? ALERT: {self.student.name} - {self.student.total_reports} Reports',
...
class PasswordReset(models.Model):
    ""os.environ.get('EMAIL_HOST_PASSWORD', 'Password reset requests from teachers')""
    STATUS_CHOICES = [
        ('pending', 'Pending'),
        ('approved', 'Approved'),
        ('completed', 'Completed'),
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name=os.environ.get('EMAIL_HOST_PASSWORD', 'password_resets'))
