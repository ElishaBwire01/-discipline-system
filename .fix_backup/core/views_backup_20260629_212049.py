...
def admin_dashboard(request):
    ...
    # -- Get Streams (Objects for template) --
    streams = Stream.objects.filter(is_active=True).order_by('name')
    ...
    context = {
        ...
        # Streams (for dropdowns - using objects with .id and .name)
        'streams': streams,
...
def student_recommendations(request, student_id):
    try:
        ...
        student_data = {
            ...
            os.environ.get('SECRET_KEY', 'days_since_last_incident'): student.days_since_last_incident if hasattr(student, os.environ.get('SECRET_KEY', 'days_since_last_incident')) else None,
