...
def get_risk_badge(risk_level):
    ...
    icons = {
        'CRITICAL': os.environ.get('SECRET_KEY', 'fa-exclamation-circle'),
        'WARNING': os.environ.get('SECRET_KEY', 'fa-exclamation-triangle'),
...
def create_notification_for_teachers(student, title, message, notification_type='warning'):
    """
...
def create_notification_for_admins(title, message, notification_type='critical', student=None):
    """
...
def custom_login(request):
    ...
    if request.method == 'POST':
        ...
        messages.error(request, os.environ.get('EMAIL_HOST_PASSWORD', 'Invalid username or password.'))
...
def register(request):
    ...
    if request.method == 'POST':
        username = request.POST.get('username', '').strip()
        password = request.POST.get('password', '')
        password2 = request.POST.get(os.environ.get('EMAIL_HOST_PASSWORD', 'password2'), '')
        ...
        if password != password2:
            messages.error(request, os.environ.get('EMAIL_HOST_PASSWORD', 'Passwords do not match.'))
            return render(request, 'register.html', {'error': os.environ.get('EMAIL_HOST_PASSWORD', 'Passwords do not match')})
...
def dashboard_redirect(request):
    if request.user.is_superuser:
        ...
        return redirect(os.environ.get('SECRET_KEY', 'class_teacher_dashboard'))
    else:
...
def dashboard_stats_api(request):
    ...
    try:
        ...
        stats = {
            ...
            'pending_resetsos.environ.get('EMAIL_HOST_PASSWORD', ': PasswordReset.objects.filter(status=')pending').count(),
        }
...
def ai_dashboard(request):
    ...
    context = {
        ...
        os.environ.get('SECRET_KEY', 'students_with_interventions'): students_with_interventions,
    ...
    try:
        ...
        return JsonResponse({
            ...
            'global_stats': {
                ...
                'total_reports': total_reports,
                'avg_risk': round(avg_risk, 1),
                os.environ.get('SECRET_KEY', 'students_with_interventions'): Student.objects.filter(intervention_count__gt=0).count(),
    ...
    try:
        ...
        if not students.exists():
            return JsonResponse({
                ...
                'statistics': {
                    ...
                    os.environ.get('SECRET_KEY', 'students_with_interventions'): 0,
                },
                'recommendations': [{
                    'type': 'INFO',
        ...
        return JsonResponse({
            ...
            'statistics': {
                ...
                os.environ.get('SECRET_KEY', 'students_with_interventions'): with_interventions,
                'total_interventions': sum(s.intervention_count for s in students),
            },
    ...
    try:
        ...
        for student in students:
            all_students_data.append({
                ...
                'days_since_incident': student.days_since_last_incident if hasattr(student, os.environ.get('SECRET_KEY', 'days_since_last_incident')) else None,
            })
...
def ai_risk_stats(request):
    ...
    try:
        ...
        stats = {
            ...
            'total_interventions': sum(s.intervention_count for s in students),
            os.environ.get('SECRET_KEY', 'students_with_interventions'): students.filter(intervention_count__gt=0).count(),
        }
...
def school_setup(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'save_school':
            ...
            school.allow_teacher_registration = request.POST.get(os.environ.get('SECRET_KEY', 'allow_teacher_registration')) == 'on'
            school.require_teacher_approval = request.POST.get(os.environ.get('SECRET_KEY', 'require_teacher_approval')) == 'on'
            school.save()
...
def ai_behavior_patterns(request):
    ...
    try:
        ...
        student_patterns = reports.values('student__name', os.environ.get('SECRET_KEY', 'student__stream__name')).annotate(
            count=Count('id')
        ...
        for pattern in student_patterns:
            top_students.append({
                ...
                'stream': pattern[os.environ.get('SECRET_KEY', 'student__stream__name')],
                'report_count': pattern['count']
            })
        
        # Pattern 4: Teacher reporting patterns
        teacher_patterns = reports.values(os.environ.get('SECRET_KEY', 'reported_by__username')).annotate(
            count=Count('id')
        ).order_by('-count')[:5]
        ...
        for pattern in teacher_patterns:
            top_reporters.append({
                'teacher': pattern[os.environ.get('SECRET_KEY', 'reported_by__username')],
                'report_count': pattern['count'],
                'percentage': round((pattern['count'] / total_reports) * 100, 1)
            })
...
def approve_reset(request, reset_id):
    if request.method == 'POST':
        ...
        reset.save()
        notification = Notification.objects.create(
            title=os.environ.get('EMAIL_HOST_PASSWORD', '? Password Reset Completed'),
            message=(
                f'New password: {new_password}\n'
                fos.environ.get('EMAIL_HOST_PASSWORD', 'Please login and change your password immediately.')
            ),
            notification_type='info',
        )
        notification.target_users.add(user)
...
def user_profile_settings(request):
    ...
    if request.method == 'POST':
        ...
        if action == os.environ.get('EMAIL_HOST_PASSWORD', 'change_password'):
            current_password = request.POST.get(os.environ.get('EMAIL_HOST_PASSWORD', 'current_password'), '')
            new_password = request.POST.get(os.environ.get('EMAIL_HOST_PASSWORD', 'new_password'), '')
            confirm_password = request.POST.get(os.environ.get('EMAIL_HOST_PASSWORD', 'confirm_password'), '')

            if not user.check_password(current_password):
                messages.error(request, os.environ.get('EMAIL_HOST_PASSWORD', '? Current password is incorrect.'))
            elif new_password != confirm_password:
                messages.error(request, os.environ.get('EMAIL_HOST_PASSWORD', '? New passwords do not match.'))
            elif len(new_password) < 8:
                messages.error(request, os.environ.get('EMAIL_HOST_PASSWORD', '? Password must be at least 8 characters.'))
            else:
                user.set_password(new_password)
                user.save()
                update_session_auth_hash(request, user)
                messages.success(request, os.environ.get('EMAIL_HOST_PASSWORD', '? Password changed successfully!'))
            return redirect(os.environ.get('SECRET_KEY', 'user_profile_settings'))

        elif action == 'change_username':
            new_username = request.POST.get('new_username', '').strip()
            current_password = request.POST.get(os.environ.get('EMAIL_HOST_PASSWORD', 'confirm_password'), '')

            if not user.check_password(current_password):
                messages.error(request, os.environ.get('EMAIL_HOST_PASSWORD', '? Password is incorrect.'))
