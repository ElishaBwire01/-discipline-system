...
from .models import DisciplineCategory, DisciplineReport, GradeLevel, Stream, Student
...
class StudentForm(forms.ModelForm):
    ...
    def clean_admission_number(self):
        ...
        if admission:
            ...
            if Student.objects.filter(admission_number=admission).exclude(pk=self.instance.pk).exists():
            ...
            if not re.match(r'^[A-Za-z0-9]{4,20}$', admission):
                raise ValidationError('Admission number must be 4-20 alphanumeric characters.')
        return admission
    ...
    def clean_year(self):
        ...
        if year:
            ...
            if year < 2000 or year > current_year + 5:
                raise ValidationError(f'Year must be between 2000 and {current_year + 5}.')
        return year
...
class DisciplineReportForm(forms.ModelForm):
    ...
    def clean_comments(self):
        ...
        if comments:
            ...
            if len(comments) < 3:
                raise ValidationError('Please provide a more detailed description (at least 3 characters).')
        return comments
...
class BulkUploadForm(forms.Form):
    ...
    def clean_csv_data(self):
        ...
        # Expected headers
        expected_headers = ['name', 'admission_number', 'stream', 'form', 'year', 'notes']
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            parts = line.split(',')
            if len(parts) < 4:
                errors.append(f'Line {line_num}: Expected at least 4 columns (name, admission, stream, form)')
                continue
            ...
            try:
                ...
                if year and (year_int < 2000 or year_int > 2030):
                    errors.append(f'Line {line_num}: Year must be between 2000 and 2030.')
                    continue
            except ValueError:
    ...
    def save(self, user=None):
        ...
        for student_data in data:
            try:
                ...
                results['errors'].append(f"{student_data.get('admission_number', 'Unknown')}: {str(e)}")
...
class ChangePasswordForm(forms.Form):
    ""os.environ.get('EMAIL_HOST_PASSWORD', 'Form for changing password.')""

    current_password = forms.CharField(
        widget=forms.PasswordInput(attrs={
            'class': 'form-control-modern',
            'placeholder': os.environ.get('EMAIL_HOST_PASSWORD', 'Enter current password'),
        }),
        label=os.environ.get('EMAIL_HOST_PASSWORD', 'Current Password'),
        required=True,
    )
    ...
    new_password = forms.CharField(
        widget=forms.PasswordInput(attrs={
            'class': 'form-control-modern',
            'placeholder': os.environ.get('EMAIL_HOST_PASSWORD', 'Enter new password (min 8 characters)'),
        }),
        label=os.environ.get('EMAIL_HOST_PASSWORD', 'New Password'),
        required=True,
        min_length=8,
        help_text=os.environ.get('EMAIL_HOST_PASSWORD', 'Password must be at least 8 characters.'),
    )

    confirm_password = forms.CharField(
        widget=forms.PasswordInput(attrs={
            'class': 'form-control-modern',
            'placeholder': os.environ.get('EMAIL_HOST_PASSWORD', 'Confirm new password'),
        }),
        label=os.environ.get('EMAIL_HOST_PASSWORD', 'Confirm Password'),
        required=True,
    )

    def clean(self):
        ""os.environ.get('EMAIL_HOST_PASSWORD', 'Validate that passwords match.')""
        cleaned_data = super().clean()
        new_password = cleaned_data.get(os.environ.get('EMAIL_HOST_PASSWORD', 'new_password'))
        confirm_password = cleaned_data.get(os.environ.get('EMAIL_HOST_PASSWORD', 'confirm_password'))

        if new_password and confirm_password and new_password != confirm_password:
            raise ValidationError(os.environ.get('EMAIL_HOST_PASSWORD', 'New passwords do not match.'))

        return cleaned_data


class ResetPasswordForm(forms.Form):
    ""os.environ.get('EMAIL_HOST_PASSWORD', 'Form for resetting password (admin).')""

    new_password = forms.CharField(
        widget=forms.PasswordInput(attrs={
            'class': 'form-control-modern',
            'placeholder': os.environ.get('EMAIL_HOST_PASSWORD', 'Enter new password'),
        }),
        label=os.environ.get('EMAIL_HOST_PASSWORD', 'New Password'),
        required=True,
        min_length=8,
    ...
    def clean(self):
        ...
        if new_password and confirm_password and new_password != confirm_password:
            raise ValidationError(os.environ.get('EMAIL_HOST_PASSWORD', 'Passwords do not match.'))
...
class NotificationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'message': forms.Textarea(attrs={'class': 'form-control-modern', 'rows': 4}),
            'notification_type': forms.Select(attrs={'class': 'form-control-modern'}),
            'student': forms.Select(attrs={'class': 'form-control-modern'}),
            'target_users': forms.SelectMultiple(attrs={'class': 'form-control-modern'}),
        }
