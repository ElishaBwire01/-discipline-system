...
def _create_report(request, student, post):
    ...
    if rating not in valid_ratings:
        rating = category.default_rating
    ...
    messages.success(
        request,
        f'✅ Report submitted for {student.name}: {category.name} '
...
def class_teacher_dashboard(request):
    ...
    # Get ALL students in this class for statistics (no pagination)
    all_students = Student.objects.filter(is_active=True, stream=assigned_stream)
    
    # Get students for display (with pagination)
    ...
    total_students = Student.objects.filter(is_active=True, stream=assigned_stream).count()  # Count only students in this stream
    ...
    if request.method == 'POST':
        ...
        if action == 'report_student':
            ...
            try:
                ...
                messages.success(request, '✅ Student added to your class!')
            except Exception as e:
                messages.error(request, f'❌ Error: {str(e)}')
            return redirect('class_teacher_dashboard')
            ...
            start_index = 0
            if lines and ('name' in lines[0].lower() or 'admission' in lines[0].lower()):
                start_index = 1
                messages.info(request, 'Header row detected and skipped.')
            ...
            for i in range(start_index, len(lines)):
                ...
                if len(parts) >= 3:
                    try:
                        ...
                        if len(parts) > 3 and parts[3] and parts[3].lower() != 'year':
                            try:
                                year = int(parts[3])
                            except ValueError:
                        ...
                        if len(parts) > 4 and parts[4].lower() != 'optional_notes':
                            notes = parts[4]
                        else:
                            notes = ''

                        if not name or not admission or not form:
                            errors.append(f'Row {i + 1}: Missing required data')
                            continue
                        ...
                        if created:
                            ...
                            errors.append(f'Row {i + 1}: Student "{admission}" already exists')
            ...
            try:
                ...
                required_columns = ['name', 'admission_number', 'form']
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        notes = str(row.get('optional_notes', '')).strip() if pd.notna(row.get('optional_notes')) else ''
                        ...
                        if created:
                            ...
                            errors.append(f'Row {index + 2}: Student already exists')
...
def student_profile(request, student_id):
    ...
    category_breakdown = reports.values(
        'category__name'
    ).annotate(count=Count('id'), total_points=Sum('points'))
...
def online_teachers_api(request):
    ...
    data = [{
        ...
        'assigned_stream': t.assigned_stream.get_name_display() if t.assigned_stream else 'Not Assigned',
