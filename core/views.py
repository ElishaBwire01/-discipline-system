...
def _create_report(request, student, post):
    """Shared helper used by both teacher and class-teacher report actions."""
...
def class_teacher_dashboard(request):
    ...
    my_students = Student.objects.filter(is_active=True, stream=assigned_stream)
    if assigned_form:
        my_students = my_students.filter(form=assigned_form)

    total_students    = my_students.count()
    critical_count    = my_students.filter(risk_score__gte=60).count()
    warning_count     = my_students.filter(risk_score__range=(30, 59)).count()
    good_count        = my_students.filter(risk_score__lt=30).count()
    ...
    my_students_display = my_students.select_related('stream')[:100]
        ...
        if action == 'report_student':
            ...
            for i in range(start_index, len(lines)):
                ...
                if len(parts) >= 3:
                    ...
                    errors.append(f'Row {i + 1}: Invalid format - need at least: name, admission_number, form')
            
            if added > 0:
                messages.success(request, f'✅ Successfully added {added} students to your class!')
            if errors:
            ...
            if added == 0 and not errors:
                messages.warning(request, 'No new students were added. Check your data format.')
            
            return redirect('class_teacher_dashboard')

    context = {
        'my_students':       my_students_display,
...
def view_user_profile(request, user_id):
    ...
    try:
        ...
        # Allow access if:
        # 1. User is superuser (admin)
        # 2. User is a teacher (has Teacher or ClassTeacher group)
