...
def global_recommendations(request):
    try:
        ...
        if warning_count > 0:
            ...
            recommendations.append({
                'action': f'⚠️ {warning_count} Students at Warning Level',
...
def class_recommendations(request, class_id):
    try:
        ...
        if critical > 0:
            ...
            recommendations.append({
                'type': 'URGENT',
                'action': f'⚠️ {critical} students at CRITICAL risk in {stream.name}',
...
def ai_all_students(request):
    try:
        ...
        if warning > 0:
            ...
            insights.append({
                'type': 'info',
                'title': f'⚠️ {warning} Students at Warning Level',
    ...
    if request.method == 'POST':
        ...
        if action == 'save_school':
            ...
            if student_count > 0:
                ...
                messages.warning(
                    request,
                    f'⚠️ {student_count} students had "{grade_name}" – grade removed from their records.'
                )
            grade.delete()
            ...
            if student_count > 0:
                ...
                messages.warning(
                    request,
                    f'⚠️ {student_count} students in "{stream_name}" were deleted along with the stream.'
                )
            stream.delete()
...
def ai_intervention_effectiveness(request):
    ...
    try:
        ...
        if total_improved > total_worsened:
            ...
            recommendations.append({
                'type': 'warning',
                'title': f'⚠️ Interventions need review ({total_worsened} students worsened)',
        ...
        for student_data in effectiveness_data[:5]:
            if student_data['effectiveness'] == 'Needs Review':
                ...
                recommendations.append({
                    'type': 'warning',
                    'title': f'⚠️ Monitor: {student_data["name"]}',
